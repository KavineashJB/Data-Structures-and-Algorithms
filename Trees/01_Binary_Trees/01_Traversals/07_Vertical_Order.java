...
class Solution {
    ...
    public void dfs(int row, int col, TreeNode node){
        if(node==null) return;
        hm.computeIfAbsent(col, k-> new TreeMap<>())
    ...
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        for(Map.Entry<Integer,TreeMap<Integer, PriorityQueue<Integer>>> e1: hm.entrySet()){
            List<Integer> temp = new ArrayList<>();
            for(Map.Entry<Integer, PriorityQueue<Integer>> e2:e1.getValue().entrySet()){
                while(!e2.getValue().isEmpty()) {
                    temp.add(e2.getValue().poll());
                }
            }
            res.add(temp);
        }
